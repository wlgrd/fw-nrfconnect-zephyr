...
static void process(struct device *dev)
{
	...
	/* Gives interrupt whenever data is ready */
	struct sensor_trigger sensor_trig_conf_drdy = {
		.type = SENSOR_TRIG_DATA_READY,
		.chan = SENSOR_CHAN_RED,
	};

	/* Gives interrupt whenever threshold value is reached */
	struct sensor_trigger sensor_trig_conf_th = {
	...
	if (IS_ENABLED(CONFIG_BH1749_TRIGGER)) {
		...
		/* For the second parameter: choose one of the sensor_trigger structs
		 * defined at the beginning of this function
		 */
		if (sensor_trigger_set(dev, &sensor_trig_conf_th, trigger_handler)) {
